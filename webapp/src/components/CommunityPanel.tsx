...
const CommunityPanel: React.FC<Props> = ({onClose}) => {
    ...
    if (loading && !me) {
        return <div data-testid='community-admin-loading' style={{padding: 16}}>Loading community admin...</div>;
    }
    ...
    return (
        <div data-testid='community-admin-panel' style={{padding: 16, maxWidth: 900, margin: '0 auto'}}>
            <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                ...
                <button type='button' data-testid='community-admin-close' onClick={onClose}>Close</button>
            </div>
            {error && <div data-testid='community-admin-error' style={{color: 'crimson', marginBottom: 12}}>{error}</div>}
            <div style={{display: 'flex', gap: 8, marginBottom: 12}}>
                <input
                    ...
                    style={{flex: 1}}
                />
                <button type='button' data-testid='community-admin-search-btn' onClick={load}>Search</button>
                {me?.permissions.create_user && (
                    <button type='button' data-testid='community-admin-create-toggle' onClick={() => setShowCreate(!showCreate)}>Create user</button>
                )}
            </div>
            {showCreate && (
            ...
            {credentialText && (
                <div data-testid='community-admin-credentials' style={{background: '#f6f6f6', padding: 12, marginBottom: 12}}>
                    <strong>Credential handoff (copy now — not stored):</strong>
                    <pre data-testid='community-admin-credentials-text' style={{whiteSpace: 'pre-wrap'}}>{credentialText}</pre>
                    <button type='button' onClick={() => navigator.clipboard.writeText(credentialText)}>Copy</button>
                    <button type='button' onClick={() => setCredentialText('')} style={{marginLeft: 8}}>Dismiss</button>
                </div>
            )}
            <table data-testid='community-admin-user-table' style={{width: '100%', borderCollapse: 'collapse'}}>
                <thead>
                    <tr>
                ...
                <tbody>
                    {users.map((u) => (
                        <tr key={u.id} data-testid={`community-admin-user-row-${u.username}`} style={{borderTop: '1px solid #ddd'}}>
                            <td>{u.username}</td>
                            <td>{u.first_name} {u.last_name}</td>
                            <td>
                                {me?.permissions.reset_password && (
                                    <button type='button' data-testid={`community-admin-reset-${u.username}`} onClick={() => resetPassword(u.id)} style={{marginRight: 8}}>Reset password</button>
                                )}
                                {me?.permissions.remove_from_team && me?.teams?.[0] && (
                                    <button type='button' data-testid={`community-admin-remove-${u.username}`} onClick={() => removeFromTeam(u.id, me.teams[0].id)}>Remove from {me.teams[0].name}</button>
                                )}
                            </td>
                        </tr>
                    ))}
